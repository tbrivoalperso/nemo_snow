...
ENDDO
!
IF(YSNOWRES=='RIL' .OR. YSNOWRES=='DEF') THEN
!
  DO JI=1,INI
!
     ZCDN(JI) = 0.0014 !(XKARMAN/ZWORK1(JI))**2
...
!
     ...
     ELSE
        IF (PRI(JI)<0.2) THEN
           ZMARTIN(JI) = MAX(0.75,( 1. - 5.*PRI(JI))*(1. - 5. *PRI(JI)))!
        ...
        ELSE
           ZMARTIN(JI)=MIN(MAX(0.75,( 1. - 5.*PRI(JI))*(1. - 5. *PRI(JI))),0.75)
        ENDIF
     ENDIF
!    
