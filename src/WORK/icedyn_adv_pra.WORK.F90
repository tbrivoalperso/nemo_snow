MODULE icedyn_adv_pra
   ...
   ! Moments for advection required only if ISBA-ES coupling
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:,:) ::   sxrhos , syrhos , sxxrhos , syyrhos , sxyrhos    ! snow layers density
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::   sxages, syages, sxxages, syyages, sxyages   ! snow age 
...
CONTAINS
   ...
   SUBROUTINE ice_dyn_adv_pra(         kt, pu_ice, pv_ice, ph_i, ph_s, ph_ip,  &
      ...
      ! Additional variables for isba-es coupling
      REAL(wp), DIMENSION(:,:,:,:), INTENT(inout) ::   prho_s       ! snw heat content
      REAL(wp), DIMENSION(:,:,:)  , INTENT(inout) ::   poa_s      ! age content
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpl)        ::   z0os ! for ISBA-ES coupling
      REAL(wp), DIMENSION(jpi,jpj,jpl)        ::   z0ap , z0vp, z0vl
      REAL(wp), DIMENSION(jpi,jpj,nlay_s,jpl) ::   z0es
      REAL(wp), DIMENSION(jpi,jpj,nlay_s,jpl) ::   z0rhos ! for ISBA-ES coupling
      ...
      DO jt = 1, icycle
         ...
         DO jl = 1, jpl
            ...
            IF( ln_isbaes) z0os (:,:,jl) = poa_s(:,:,jl) * e1e2t(:,:)        ! Snow Age content

            DO jk = 1, nlay_s
               z0es(:,:,jk,jl) = pe_s(:,:,jk,jl) * e1e2t(:,:) ! Snow heat content
               IF( ln_isbaes) z0rhos(:,:,jk,jl) = prho_s(:,:,jk,jl) * e1e2t(:,:) ! Snow heat content
            END DO
            DO jk = 1, nlay_i
         ...
         IF( MOD( (kt - 1) / nn_fsbc , 2 ) ==  MOD( (jt - 1) , 2 ) ) THEN   !==  odd ice time step:  adv_x then adv_y  ==!
            ...
            END DO
            IF( ln_isbaes) THEN
               CALL adv_x( zdt , zudy , 1._wp , zarea , z0os  , sxages , sxxages , syages , syyages , sxyages ) !--- snow ages
               CALL adv_y( zdt , zvdx , 0._wp , zarea , z0os  , sxages , sxxages , syages , syyages , sxyages )

               DO jk = 1, nlay_s                                                                           !--- snow density
                  CALL adv_x( zdt, zudy, 1._wp, zarea, z0rhos (:,:,jk,:), sxrhos(:,:,jk,:),   &
                     &                                 sxxrhos(:,:,jk,:), syrhos(:,:,jk,:), syyrhos(:,:,jk,:), sxyrhos(:,:,jk,:) )
                  CALL adv_y( zdt, zvdx, 0._wp, zarea, z0rhos (:,:,jk,:), sxrhos(:,:,jk,:),   &
                     &                                 sxxrhos(:,:,jk,:), syrhos(:,:,jk,:), syyrhos(:,:,jk,:), sxyrhos(:,:,jk,:) )
               END DO
            ENDIF        
         ...
         DO jl = 1, jpl
            ...
            IF( ln_isbaes) poa_s(:,:,jl) = z0os (:,:,jl) * r1_e1e2t(:,:) * tmask(:,:,1)

            pa_i (:,:,jl) = z0ai (:,:,jl) * r1_e1e2t(:,:) * tmask(:,:,1)
             
            DO jk = 1, nlay_s
               pe_s(:,:,jk,jl) = z0es(:,:,jk,jl) * r1_e1e2t(:,:) * tmask(:,:,1)
               IF( ln_isbaes) prho_s(:,:,jk,jl) = z0rhos(:,:,jk,jl) * r1_e1e2t(:,:) * tmask(:,:,1)
