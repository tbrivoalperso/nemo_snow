MODULE icedyn_adv_umx_isbaes
   ...
   !!----------------------------------------------------------------------
   !!   ice_dyn_adv_umx_isbaes   : update the tracer fields
   ...
   PUBLIC   ice_dyn_adv_umx_isbaes   ! called by icedyn_adv.F90
   !
...
#  include "do_loop_substitute.h90"
   ...
   !! $Id: icedyn_adv_umx_isbaes.F90 15049 2021-06-23 16:17:30Z clem $
...
CONTAINS
   ...
   SUBROUTINE ice_dyn_adv_umx_isbaes( kn_umx, kt, pu_ice, pv_ice, ph_i, ph_s, ph_ip,  &
      ...
      REAL(wp), DIMENSION(jpi,jpj,nlay_s,jpl) ::   zrhov_s, zrhovs_max
      REAL(wp), DIMENSION(jpi,jpj,nlay_s,jpl) ::   zdv_s, zdvs_max
      !
      ...
      !!----------------------------------------------------------------------
      !
      IF( kt == nit000 .AND. lwp )   WRITE(numout,*) '-- ice_dyn_adv_umx_isbaes: Ultimate-Macho advection scheme'
      !
      ...
      CALL lbc_lnk( 'icedyn_adv_umx_isbaes', zhi_max, 'T', 1._wp, zhs_max, 'T', 1._wp, zhip_max, 'T', 1._wp, zsi_max, 'T', 1._wp )
      !
      ! enthalpies
      DO jk = 1, nlay_i
      ...
      CALL lbc_lnk( 'icedyn_adv_umx_isbaes', zei_max, 'T', 1._wp )
      CALL lbc_lnk( 'icedyn_adv_umx_isbaes', zes_max, 'T', 1._wp )
      !
      !
      ...
      CALL mpp_delay_max( 'icedyn_adv_umx_isbaes', 'cflice', zcflnow(:), zcflprv(:), kt == nitend - nn_fsbc + 1 )
      ...
      DO jt = 1, icycle
         ...
         IF( np_advS == 1 ) THEN       ! -- advection form: -div( uVS ) -- !
            ...
            !== Snw volume ==!
            DO jk = 1, nlay_s
               zhvar(:,:,:) = pdv_s(:,:,jk,:) * z1_ai(:,:,:)
               CALL adv_umx( zamsk, kn_umx, jt, kt, zdt, zudy , zvdx, zua_ho , zva_ho , zcu_box, zcv_box, &
                  &                                      zhvar, pdv_s(:,:,jk,:), zua_ups, zva_ups )
            ENDDO
         ...
         ELSEIF( np_advS == 2 ) THEN   ! -- advection form: -div( uA * uHS / u ) -- !
            ...
            DO jk = 1, nlay_s
               !== Snw mass ==!
               zhvar(:,:,:) = prhov_s(:,:,jk,:) * z1_ai(:,:,:)
               CALL adv_umx( zamsk, kn_umx, jt, kt, zdt, zudy , zvdx, zua_ho , zva_ho , zcu_box, zcv_box, &
                  &                                      zhvar, prhov_s(:,:,jk,:), zua_ups, zva_ups )
            END DO
         ...
         IF    ( ( ln_pnd_LEV .OR. ln_pnd_TOPO ) .AND. ln_pnd_lids ) THEN
            CALL lbc_lnk( 'icedyn_adv_umx_isbaes', pa_i,'T',1._wp, pv_i,'T',1._wp, pv_s,'T',1._wp, psv_i,'T',1._wp, poa_i,'T',1._wp &
         ...
         ELSE
            CALL lbc_lnk( 'icedyn_adv_umx_isbaes', pa_i,'T',1._wp, pv_i,'T',1._wp, pv_s,'T',1._wp, psv_i,'T',1._wp, poa_i,'T',1._wp )
         ENDIF
         CALL lbc_lnk( 'icedyn_adv_umx_isbaes', pe_i, 'T', 1._wp )
         CALL lbc_lnk( 'icedyn_adv_umx_isbaes', pe_s, 'T', 1._wp )
         !
         !== Open water area ==!
         ...
         END_2D
         CALL lbc_lnk( 'icedyn_adv_umx_isbaes', pato_i, 'T',  1._wp )
         !
         ! --- diagnostics --- !
      ...
      END DO
      !
   END SUBROUTINE ice_dyn_adv_umx_isbaes
   ...
   SUBROUTINE adv_umx( pamsk, kn_umx, jt, kt, pdt, pu, pv, puc, pvc, pubox, pvbox,  &
      ...
      IF( pamsk == 0._wp ) THEN
         ...
         DO jl = 1, jpl
            ...
            END_2D
         END DO
         CALL lbc_lnk( 'icedyn_adv_umx_isbaes', zt_ups, 'T',  1.0_wp )
         !
   ...
   SUBROUTINE upstream( pamsk, jt, kt, pdt, pt, pu, pv, pt_ups, pfu_ups, pfv_ups )
      ...
      DO jl = 1, jpl                    !-- after tracer with upstream scheme
         ...
         END_2D
      END DO
      IF( nn_hls == 1 )   CALL lbc_lnk( 'icedyn_adv_umx_isbaes', pt_ups, 'T', 1.0_wp )
...
!!$         END DO
      END DO
      IF( nn_hls == 1 )   CALL lbc_lnk( 'icedyn_adv_umx_isbaes', ztu2, 'T', 1.0_wp )
      !
...
!!rachid          zdx2 = e1u(ji,jj) * e1t(ji,jj)
      ...
      CASE( 5 )                                                   !==  5th order central TIM  ==! (Eq. 29)
         !
         CALL lbc_lnk( 'icedyn_adv_umx_isbaes', ztu4, 'T', 1.0_wp )
         !
         DO jl = 1, jpl
            DO_2D( 1, 0, kloop, kloop )
               zcu  = pu(ji,jj) * r1_e2u(ji,jj) * pdt * r1_e1u(ji,jj)
...
!!rachid          zdx2 = e1u(ji,jj) * e1t(ji,jj)
   ...
   SUBROUTINE ultimate_y( kloop, pamsk, kn_umx, pdt, pt, pv, pt_v, pfv_ho )
      ...
      DO jl = 1, jpl
         ...
         END_2D
      END DO
      IF( nn_hls == 1 )   CALL lbc_lnk( 'icedyn_adv_umx_isbaes', ztv2, 'T', 1.0_wp )
      !
...
!!rachid          zdy2 = e2v(ji,jj) * e2t(ji,jj)
      ...
      CASE( 5 )                                                !==  5th order central TIM  ==! (Eq. 29)
         !
         CALL lbc_lnk( 'icedyn_adv_umx_isbaes', ztv4, 'T', 1.0_wp )
         !
         DO jl = 1, jpl
            DO_2D( kloop, kloop, 1, 0 )
               zcv  = pv(ji,jj) * r1_e1v(ji,jj) * pdt * r1_e2v(ji,jj)
...
!!rachid          zdy2 = e2v(ji,jj) * e2t(ji,jj)
   ...
   SUBROUTINE nonosc_ice( pamsk, pdt, pu, pv, pt, pt_ups, pfu_ups, pfv_ups, pfu_ho, pfv_ho )
      ...
      IF( ll_prelim ) THEN
         ...
         DO jl = 1, jpl
            ...
            END_2D
         END DO
         CALL lbc_lnk( 'icedyn_adv_umx_isbaes', zti_ups, 'T', 1.0_wp, ztj_ups, 'T', 1.0_wp )
         ...
         DO jl = 1, jpl
            ...
            END_2D
         END DO
         CALL lbc_lnk( 'icedyn_adv_umx_isbaes', pfu_ho, 'U', -1.0_wp, pfv_ho, 'V', -1.0_wp )   ! lateral boundary cond.
      ...
      DO jl = 1, jpl
         ...
         END_2D
      END DO
      CALL lbc_lnk( 'icedyn_adv_umx_isbaes', zbetup, 'T', 1.0_wp, zbetdo, 'T', 1.0_wp )   ! lateral boundary cond. (unchanged sign)
   ...
   SUBROUTINE limiter_x( pdt, pu, pt, pfu_ups, pfu_ho )
      ...
      DO jl = 1, jpl
         ...
         END_2D
      END DO
      IF( nn_hls == 1 )   CALL lbc_lnk( 'icedyn_adv_umx_isbaes', zslpx, 'U', -1.0_wp)   ! lateral boundary cond.
      ...
      DO jl = 1, jpl
         ...
         END_2D
      END DO
      IF( nn_hls == 1 )   CALL lbc_lnk( 'icedyn_adv_umx_isbaes', pfu_ho, 'U', -1.0_wp)   ! lateral boundary cond.
      !
   ...
   SUBROUTINE limiter_y( pdt, pv, pt, pfv_ups, pfv_ho )
      ...
      DO jl = 1, jpl
         ...
         END_2D
      END DO
      IF( nn_hls == 1 )   CALL lbc_lnk( 'icedyn_adv_umx_isbaes', zslpy, 'V', -1.0_wp)   ! lateral boundary cond.
      ...
      DO jl = 1, jpl
         ...
         END_2D
      END DO
      IF( nn_hls == 1 )   CALL lbc_lnk( 'icedyn_adv_umx_isbaes', pfv_ho, 'V', -1.0_wp)   ! lateral boundary cond.
      !
