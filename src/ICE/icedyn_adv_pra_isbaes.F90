MODULE icedyn_adv_pra_isbaes
   ...
   !!----------------------------------------------------------------------
   !!   ice_dyn_adv_pra_isbaes : advection of sea ice using Prather scheme
   !!   adv_x, adv_y    : Prather scheme applied in i- and j-direction, resp.
   !!   adv_pra_isbaes_init    : initialisation of the Prather scheme
   !!   adv_pra_isbaes_rst     : read/write Prather field in ice restart file, or initialized to zero
   !!----------------------------------------------------------------------
   ...
   PUBLIC   ice_dyn_adv_pra_isbaes   ! called by icedyn_adv
   PUBLIC   adv_pra_isbaes_init      ! called by icedyn_adv
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:,:) ::   sxdvs , sydvs , sxxdvs , syydvs , sxydvs    ! snow layers volume
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:,:) ::   sxrhovs , syrhovs , sxxrhovs , syyrhovs , sxyrhovs    ! snow layers mass
...
#  include "do_loop_substitute.h90"
   ...
   !! $Id: icedyn_adv_pra_isbaes.F90 15049 2021-06-23 16:17:30Z clem $
...
CONTAINS
   ...
   SUBROUTINE ice_dyn_adv_pra_isbaes(         kt, pu_ice, pv_ice, ph_i, ph_s, ph_ip,  &
      ...
      !!----------------------------------------------------------------------
      !
      IF( kt == nit000 .AND. lwp )   WRITE(numout,*) '-- ice_dyn_adv_pra_isbaes: Prather advection scheme'
      !
      ...
      CALL lbc_lnk( 'icedyn_adv_pra_isbaes', zhi_max, 'T', 1._wp, zhs_max, 'T', 1._wp, zhip_max, 'T', 1._wp, zsi_max, 'T', 1._wp )
      !
      ! enthalpies
      DO jk = 1, nlay_i
      ...
      CALL mpp_delay_max( 'icedyn_adv_pra_isbaes', 'cflice', zcflnow(:), zcflprv(:), kt == nitend - nn_fsbc + 1 )
      ...
      DO jt = 1, icycle
         ...
         ! diagnostics
         zdiag_adv_mass(:,:) =   SUM( pv_i (:,:,:) , dim=3 ) * rhoi + SUM(SUM( prhov_s (:,:,:,:) , dim=3 ), dim=3 )  &
         ...
         IF( MOD( (kt - 1) / nn_fsbc , 2 ) ==  MOD( (jt - 1) , 2 ) ) THEN   !==  odd ice time step:  adv_x then adv_y  ==!
            ...
            DO jk = 1, nlay_s                                                                           !--- snow heat content
               ...
               CALL adv_y( zdt, zvdx, 0._wp, zarea, z0es (:,:,jk,:), sxc0(:,:,jk,:),   &
                  &                                 sxxc0(:,:,jk,:), syc0(:,:,jk,:), syyc0(:,:,jk,:), sxyc0(:,:,jk,:) )
                                                                                                        !--- snow volume
               CALL adv_x( zdt, zudy, 1._wp, zarea, z0dvs (:,:,jk,:), sxdvs(:,:,jk,:),   &
                  &                                 sxxdvs(:,:,jk,:), sydvs(:,:,jk,:), syydvs(:,:,jk,:), sxydvs(:,:,jk,:) )
               CALL adv_y( zdt, zvdx, 0._wp, zarea, z0dvs (:,:,jk,:), sxdvs(:,:,jk,:),   &
                  &                                 sxxdvs(:,:,jk,:), sydvs(:,:,jk,:), syydvs(:,:,jk,:), sxydvs(:,:,jk,:) )

                                                                                                        !--- snow mass
               CALL adv_x( zdt, zudy, 1._wp, zarea, z0rhovs (:,:,jk,:), sxrhovs(:,:,jk,:),   &
         ...
         ELSE                                                               !== even ice time step:  adv_y then adv_x  ==!
            ...
            DO jk = 1, nlay_s                                                                           !--- snow heat content
               ...
               CALL adv_x( zdt, zudy, 0._wp, zarea, z0es (:,:,jk,:), sxc0(:,:,jk,:),   &
                  &                                 sxxc0(:,:,jk,:), syc0(:,:,jk,:), syyc0(:,:,jk,:), sxyc0(:,:,jk,:) )
                                                                                                        !--- snow volume
               CALL adv_y( zdt, zvdx, 1._wp, zarea, z0dvs (:,:,jk,:), sxdvs(:,:,jk,:),   &
                  &                                 sxxdvs(:,:,jk,:), sydvs(:,:,jk,:), syydvs(:,:,jk,:), sxydvs(:,:,jk,:) )
               CALL adv_x( zdt, zudy, 0._wp, zarea, z0dvs (:,:,jk,:), sxdvs(:,:,jk,:),   &
                  &                                 sxxdvs(:,:,jk,:), sydvs(:,:,jk,:), syydvs(:,:,jk,:), sxydvs(:,:,jk,:) )

                                                                                                        !--- snow mass
               CALL adv_y( zdt, zvdx, 1._wp, zarea, z0rhovs (:,:,jk,:), sxrhovs(:,:,jk,:),   &
                  &                                 sxxrhovs(:,:,jk,:), syrhovs(:,:,jk,:), syyrhovs(:,:,jk,:), sxyrhovs(:,:,jk,:) )
         ...
         CALL lbc_lnk( 'icedyn_adv_pra_isbaes', z0ice , 'T', 1._wp, sxice , 'T', -1._wp, syice , 'T', -1._wp  & ! ice volume
         ...
         CALL lbc_lnk( 'icedyn_adv_pra_isbaes', z0es  , 'T', 1._wp, sxc0  , 'T', -1._wp, syc0  , 'T', -1._wp  & ! snw enthalpy
            &                          , sxxc0 , 'T', 1._wp, syyc0 , 'T',  1._wp, sxyc0 , 'T',  1._wp  )
         CALL lbc_lnk( 'icedyn_adv_pra_isbaes', z0dvs  , 'T', 1._wp, sxdvs  , 'T', -1._wp, sydvs  , 'T', -1._wp  & ! snw volume 
            &                          , sxxdvs , 'T', 1._wp, syydvs , 'T',  1._wp, sxydvs , 'T',  1._wp  )
         ...
         IF ( ln_pnd_LEV .OR. ln_pnd_TOPO ) THEN
            IF( ln_pnd_lids ) THEN
               CALL lbc_lnk( 'icedyn_adv_pra_isbaes', z0ap , 'T', 1._wp, sxap , 'T', -1._wp, syap , 'T', -1._wp  & ! melt pond fraction
         ...
         END_2D
         CALL lbc_lnk( 'icedyn_adv_pra_isbaes', pato_i, 'T',  1.0_wp )
         !
         ! --- diagnostics --- !
         diag_adv_mass(:,:) = diag_adv_mass(:,:) + (   SUM( pv_i (:,:,:) , dim=3 ) * rhoi + SUM(SUM( prhov_s (:,:,:,:) , dim=3 ), dim=3 ) &
            &                                        + SUM( pv_ip(:,:,:) , dim=3 ) * rhow + SUM( pv_il(:,:,:) , dim=3 ) * rhow &
      ...
      DO jl = 1, jpl
         ...
         DO_2D( 0, 0, 0, 0 )
            ...
            IF((ABS(glamt(ji,jj) - (14.0005165608618)) .lt. 1e-4) .AND.  (ABS(gphit(ji,jj) - (-54.9237748235416)) .lt. 1e-4) ) THEN
                PRINT*,'                   '
                PRINT*,'                   '
                PRINT*,'                   '
                PRINT*,'TS=',kt,'AFTER ADV'
                PRINT*,'jl',jl
      ...
      END DO
         ...
         !CALL Hsnow_isbaes( zdt, pv_i, pv_s, pa_i, pa_ip, pe_s, zrho_s, pdv_s ) ! We avoid this for now, since it has to be rewrited!
         !
      ...
      !
   END SUBROUTINE ice_dyn_adv_pra_isbaes
         ...
         DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
            IF ( pv_i(ji,jj,jl) > 0._wp ) THEN
               ...
               DO jk = 1, nlay_s
                  ...
                  !PRINT*,'zhs',zhs
                  !PRINT*,'pa_i',pa_i(ji,jj,jl)
                  !PRINT*,'pdv_s(ji,jj,jk,jl)',pdv_s(ji,jj,jk,jl)
                  !PRINT*,'pdhs_max(ji,jj,jk,jl)',pdhs_max(ji,jj,jk,jl)
                  ...
                  IF( pdv_s(ji,jj,jk,jl) > 0._wp .AND. zhs > pdhs_max(ji,jj,jk,jl) .AND.  pa_i(ji,jj,jl) < 0.15 ) THEN
                     ...
                     !PRINT*,'RHOMAX',prho_s(ji,jj,jk,jl)
                     pe_s(ji,jj,jk,jl) = pe_s(ji,jj,jk,jl) * zfra
                     pdv_s(ji,jj,jk,jl)          = pa_i(ji,jj,jl) * pdhs_max(ji,jj,jk,jl) 
                  ENDIF
               END DO 
               pv_s(ji,jj,jl) = SUM(pdv_s(ji,jj,:,jl))
               ...
               IF( pv_s(ji,jj,jl) > 0._wp .AND. zhs > phs_max(ji,jj,jl) .AND.  pa_i(ji,jj,jl) < 0.15 ) THEN
                  ...
                  !
                  wfx_res(ji,jj) = wfx_res(ji,jj) - SUM(prho_s(ji,jj,:,jl) * pdv_s(ji,jj,:,jl) * ( 1._wp - zfra )) * z1_dt
      ...
      DO jl = 1, jpl
         DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, nlay_s )
            IF ( pdv_s(ji,jj,jk,jl) > 0._wp ) THEN
               ...
               ! excess in the ocean
               IF( prho_s(ji,jj,jk,jl)  > prhos_max(ji,jj,jk,jl) .AND. (pdv_s(ji,jj,jk,jl) / pa_i(ji,jj,jl)) < 1e-3 ) THEN !.AND.  pa_i(ji,jj,jl) < 0.15 ) THEN
...
!      END DO
   ...
   SUBROUTINE Hsnow_isbaes( pdt, pv_i, pv_s, pa_i, pa_ip, pe_s, prho_s, pdv_s )
      !!-------------------------------------------------------------------
      ...
      REAL(wp) ::   z1_dt, zvs_excess, zfra, zdv, zden
      !!-------------------------------------------------------------------
      !
      z1_dt = 1._wp / pdt
      !
      ...
      DO jl = 1, jpl
         DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
            ...
            IF ( pv_i(ji,jj,jl) > 0._wp ) THEN
                  !
               zden = (SUM(pdv_s(ji,jj,:,jl) * prho_s(ji,jj,:,jl)) / SUM(pdv_s(ji,jj,:,jl)))  + rho0 - rhoi !  denominator
               zdv  = MAX( SUM(pdv_s(ji,jj,:,jl) * prho_s(ji,jj,:,jl)) + ( rhoi - rho0 ) * pv_i(ji,jj,jl) , 0._wp) / zden ! snow ice for layer jk 
               DO jk = nlay_s, 1, -1
                  ...
                  IF( zvs_excess > 0._wp .AND. pdv_s(ji,jj,jk,jl) > 0._wp) THEN   ! snow-ice interface deplets below the ocean surface
                     ...
                     zfra = zvs_excess / pdv_s(ji,jj,jk,jl)
                     ...
                     !pe_s(ji,jj,jk,jl) = pe_s(ji,jj,jk,jl) * zfra
                     pdv_s(ji,jj,jk,jl)          = pdv_s(ji,jj,jk,jl) - zvs_excess
                  ENDIF
                  PRINT*,'zvs_excess 1 ',zvs_excess
                  PRINT*,'zdv 1',zdv
                  PRINT*,'pdv_s(ji,jj,jk,jl) 1',pdv_s(ji,jj,jk,jl) 
                  PRINT*,'pdv_s(ji,jj,jk,jl) - zvs_excess 1',pdv_s(ji,jj,jk,jl) - zvs_excess

                  zdv = zdv - zvs_excess 
                  !
                  PRINT*,'zvs_excess 2',zvs_excess
                  PRINT*,'zdv 2',zdv
                  PRINT*,'pdv_s(ji,jj,jk,jl) 2',pdv_s(ji,jj,jk,jl) 
                  PRINT*,'pdv_s(ji,jj,jk,jl) - zvs_excess 2',pdv_s(ji,jj,jk,jl) - zvs_excess
               ENDDO
            ENDIF
         END_2D
      END DO
   ...
   SUBROUTINE adv_pra_isbaes_init
      ...
      ALLOCATE( sxice(jpi,jpj,jpl) , syice(jpi,jpj,jpl) , sxxice(jpi,jpj,jpl) , syyice(jpi,jpj,jpl) , sxyice(jpi,jpj,jpl) ,   &
         ...
         !
         &      sxdvs (jpi,jpj,nlay_s,jpl) , sydvs (jpi,jpj,nlay_s,jpl) , sxxdvs(jpi,jpj,nlay_s,jpl) , &
         &      syydvs(jpi,jpj,nlay_s,jpl) , sxydvs(jpi,jpj,nlay_s,jpl)                             , &
         !
...
!            ENDIF
      ...
      !
      CALL adv_pra_isbaes_rst( 'READ' )    !* read or initialize all required files
      !
   END SUBROUTINE adv_pra_isbaes_init


   SUBROUTINE adv_pra_isbaes_rst( cdrw, kt )
      !!---------------------------------------------------------------------
      !!                   ***  ROUTINE adv_pra_isbaes_rst  ***
      !!
      ...
      IF( TRIM(cdrw) == 'READ' ) THEN        !==  Read or initialize  ==!
         ...
         IF( id1 > 0 ) THEN                     !**  Read the restart file  **!
            ...
            DO jk = 1, nlay_s
               WRITE(zchar1,'(I2.2)') jk
               znam = 'sxdvs'//'_l'//zchar1
               CALL iom_get( numrir, jpdom_auto, znam , z3d, psgn = -1._wp )   ;   sxdvs (:,:,jk,:) = z3d(:,:,:)
               znam = 'sydvs'//'_l'//zchar1
               CALL iom_get( numrir, jpdom_auto, znam , z3d, psgn = -1._wp )   ;   sydvs (:,:,jk,:) = z3d(:,:,:)
               znam = 'sxxdvs'//'_l'//zchar1
               CALL iom_get( numrir, jpdom_auto, znam , z3d )   ;   sxxdvs(:,:,jk,:) = z3d(:,:,:)
               znam = 'syydvs'//'_l'//zchar1
               CALL iom_get( numrir, jpdom_auto, znam , z3d )   ;   syydvs(:,:,jk,:) = z3d(:,:,:)
               znam = 'sxydvs'//'_l'//zchar1
               CALL iom_get( numrir, jpdom_auto, znam , z3d )   ;   sxydvs(:,:,jk,:) = z3d(:,:,:)
            END DO
            !    
            !                                                        ! snow layers mass
            DO jk = 1, nlay_s
               WRITE(zchar1,'(I2.2)') jk
               znam = 'sxrhovs'//'_l'//zchar1
               CALL iom_get( numrir, jpdom_auto, znam , z3d, psgn = -1._wp )   ;   sxrhovs (:,:,jk,:) = z3d(:,:,:)
               znam = 'syrhovs'//'_l'//zchar1
               CALL iom_get( numrir, jpdom_auto, znam , z3d, psgn = -1._wp )   ;   syrhovs (:,:,jk,:) = z3d(:,:,:)
               znam = 'sxxrhovs'//'_l'//zchar1
               CALL iom_get( numrir, jpdom_auto, znam , z3d )   ;   sxxrhovs(:,:,jk,:) = z3d(:,:,:)
               znam = 'syyrhovs'//'_l'//zchar1
      ...
      ELSEIF( TRIM(cdrw) == 'WRITE' ) THEN   !==  write in the ice restart file  ==!
         ...
         DO jk = 1, nlay_s
            WRITE(zchar1,'(I2.2)') jk
            znam = 'sxdvs'//'_l'//zchar1  ;   z3d(:,:,:) = sxdvs (:,:,jk,:)
            CALL iom_rstput( iter, nitrst, numriw, znam , z3d )
            znam = 'sydvs'//'_l'//zchar1  ;   z3d(:,:,:) = sydvs (:,:,jk,:)
            CALL iom_rstput( iter, nitrst, numriw, znam , z3d )
            znam = 'sxxdvs'//'_l'//zchar1 ;   z3d(:,:,:) = sxxdvs(:,:,jk,:)
            CALL iom_rstput( iter, nitrst, numriw, znam , z3d )
            znam = 'syydvs'//'_l'//zchar1 ;   z3d(:,:,:) = syydvs(:,:,jk,:)
            CALL iom_rstput( iter, nitrst, numriw, znam , z3d )
            znam = 'sxydvs'//'_l'//zchar1 ;   z3d(:,:,:) = sxydvs(:,:,jk,:)
            CALL iom_rstput( iter, nitrst, numriw, znam , z3d )
         END DO
         !                                                           ! snow layers mass
         DO jk = 1, nlay_s
            WRITE(zchar1,'(I2.2)') jk
            znam = 'sxrhovs'//'_l'//zchar1  ;   z3d(:,:,:) = sxrhovs (:,:,jk,:)
            CALL iom_rstput( iter, nitrst, numriw, znam , z3d )
            znam = 'syrhovs'//'_l'//zchar1  ;   z3d(:,:,:) = syrhovs (:,:,jk,:)
            CALL iom_rstput( iter, nitrst, numriw, znam , z3d )
            znam = 'sxxrhovs'//'_l'//zchar1 ;   z3d(:,:,:) = sxxrhovs(:,:,jk,:)
            CALL iom_rstput( iter, nitrst, numriw, znam , z3d )
            znam = 'syyrhovs'//'_l'//zchar1 ;   z3d(:,:,:) = syyrhovs(:,:,jk,:)
            CALL iom_rstput( iter, nitrst, numriw, znam , z3d )
      ...
      ENDIF
      !
   END SUBROUTINE adv_pra_isbaes_rst
...
!!$      END DO
   ...
   !!======================================================================
END MODULE icedyn_adv_pra_isbaes
